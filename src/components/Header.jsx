...
import { Search, Heart, ShoppingBag, User, Bell, ShieldCheck } from "lucide-react";
import { useState } from "react";
import { useShop } from "../context/ShopContext";
import { useAuth } from "../context/AuthContext";

const NAV = ["Women", "Men", "Kids", "Beauty"];
...
export const Header = () => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-gray-200">
      <div className="w-full px-4 md:px-8 h-20 flex items-center gap-4">
        ...
        <Link to="/" className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[#282C3F] font-bold text-sm uppercase tracking-wide hover:text-[#FF3E6C] transition-colors">
          Home
        </Link>
        <nav className="hidden md:flex items-center gap-6 ml-4">
          {NAV.map((n) => (
            <Link
              key={n}
              to={n === "Women" || n === "Men" || n === "Beauty" || n === "Kids" ? `/products?gender=${n}` : "/products"}
        ...
        <div className="flex items-center gap-4 ml-auto sm:ml-0">
          ...
          {user?.role && ["operator", "manager"].includes(user.role) ? (
            <Link to="/ops" data-testid="header-ops" className="flex flex-col items-center text-[#282C3F] hover:text-[#FF3E6C] transition-colors">
              <ShieldCheck size={20} />
              <span className="text-[10px] font-semibold hidden md:block">Ops</span>
            </Link>
          ) : null}
