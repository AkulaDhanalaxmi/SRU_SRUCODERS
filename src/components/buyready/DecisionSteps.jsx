...
export const DecisionSteps = ({ state, setState, evaluation, compact = false }) => {
  ...
  const Step = ({ n, icon: Icon, title, value, cta, onClick, testId, done }) => (
    <button data-testid={testId} onClick={onClick}
      className={`w-full flex items-center gap-3 border rounded-2xl px-4 py-3 text-left transition-colors ${done ? "border-[#03A685]/40 bg-[#03A685]/[0.06]" : "border-[#e9e9ef] hover:border-[#ff3f6c]/50"}`}>
      <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${done ? "bg-[#03a685] text-white" : "bg-[#f7f7fa] text-[#535766]"}`}>
        <Icon size={16} />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#7e7e7e]">Step {n}</p>
        <p className="mt-1 text-sm font-semibold text-[#212121] truncate">{title}</p>
        <p className="text-[12px] text-[#6b7280] truncate mt-1">{value}</p>
      </div>
      <span className={`text-[11px] font-bold ${done ? "text-[#03a685]" : "text-[#ff3f6c]"}`}>{cta}</span>
    </button>
  );
  ...
  return (
    <div className={compact ? "space-y-2" : "space-y-3"}>
      <Step n={1} icon={Ruler} title="Fit Profile" testId="step-fit-profile" done={!!fp}
        value={fp ? `Using ${fp.name}'s Fit Profile${evaluation?.recommended_size ? ` · Size ${evaluation.recommended_size}` : ""}` : "Select or create a fit profile"}
      ...
      <Step n={3} icon={CalendarHeart} title="Buying Purpose" testId="step-purpose" done={!!state.purpose}
        value={state.purpose ? `${state.purpose}${state.eventDate ? ` · ${new Date(state.eventDate).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}` : ""}` : "What's the occasion?"}
      ...
      <BottomSheet open={sheet === "purpose"} onClose={() => setSheet(null)} title="What are you buying this for?" testId="purpose-sheet">
        ...
        {state.purpose && ["Wedding", "Festival", "Gift"].includes(state.purpose) && (
          <div className="mt-5">
            ...
            <input data-testid="event-date-input" type="date" value={state.eventDate || ""} min={new Date().toISOString().split("T")[0]}
              onChange={(e) => setState((s) => ({ ...s, eventDate: e.target.value }))}
              className="w-full mt-2 border border-gray-300 rounded-md px-3 py-2.5 text-sm outline-none focus:border-[#FF3E6C]" />
          </div>
        )}
